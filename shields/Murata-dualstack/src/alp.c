...
void alp_append_length_operand(fifo_t* fifo, uint32_t length) {
  if(length < 64) {
    ...
    assert(fifo_put(fifo, (uint8_t*)&length, 1) == oss7SUCCESS);
    return;
  }
  ...
  assert(fifo_put(fifo, &byte, 1) == oss7SUCCESS);
  do {
    size--;
    assert(fifo_put(fifo, (uint8_t*)&length + size, 1) == oss7SUCCESS);
  } while(size > 0);
}

alp_operand_file_offset_t alp_parse_file_offset_operand(fifo_t* cmd_fifo) {
  alp_operand_file_offset_t operand;
  error_t err = fifo_pop(cmd_fifo, &operand.file_id, 1); assert(err == oss7SUCCESS);
...
alp_operand_file_header_t alp_parse_file_header_operand(fifo_t* cmd_fifo) {
  ...
  error_t err = fifo_pop(cmd_fifo, &operand.file_id, 1); assert(err == oss7SUCCESS);
  err = fifo_pop(cmd_fifo, (uint8_t*)&operand.file_header, sizeof(d7ap_fs_file_header_t)); assert(err == oss7SUCCESS);
...
void alp_append_file_offset_operand(fifo_t* fifo, uint8_t file_id, uint32_t offset) {
  assert(fifo_put_byte(fifo, file_id) == oss7SUCCESS);
...
void alp_append_indirect_forward_action(fifo_t* fifo, uint8_t file_id, bool overload, uint8_t *overload_config, uint8_t overload_config_len) {
  assert(fifo_put_byte(fifo, ALP_OP_INDIRECT_FORWARD | (overload << 7)) == oss7SUCCESS);
  assert(fifo_put_byte(fifo, file_id) == oss7SUCCESS);

  if (overload) {
    assert(fifo_put(fifo, overload_config, overload_config_len) == oss7SUCCESS);
  }
...
void alp_append_forward_action(fifo_t* fifo, uint8_t itf_id, uint8_t *config, uint8_t config_len) {
  assert(config!=NULL);
  assert(fifo_put_byte(fifo, ALP_OP_FORWARD) == oss7SUCCESS);
  assert(fifo_put_byte(fifo, itf_id) == oss7SUCCESS);

  if (itf_id == ALP_ITF_ID_D7ASP)
  {
    assert(fifo_put_byte(fifo, ((d7ap_session_config_t*)config)->qos.raw) == oss7SUCCESS);
    assert(fifo_put_byte(fifo, ((d7ap_session_config_t*)config)->dormant_timeout) == oss7SUCCESS);
    assert(fifo_put_byte(fifo, ((d7ap_session_config_t*)config)->addressee.ctrl.raw) == oss7SUCCESS);
    uint8_t id_length = d7ap_addressee_id_length(((d7ap_session_config_t*)config)->addressee.ctrl.id_type);
    assert(fifo_put_byte(fifo, ((d7ap_session_config_t*)config)->addressee.access_class) == oss7SUCCESS);
    assert(fifo_put(fifo, ((d7ap_session_config_t*)config)->addressee.id, id_length) == oss7SUCCESS);
  }
  else if(itf_id == ALP_ITF_ID_LORAWAN_ABP)
  {
    ...
    assert(fifo_put_byte(fifo, control_byte) == oss7SUCCESS);
    assert(fifo_put_byte(fifo, ((lorawan_session_config_abp_t*)config)->application_port) == oss7SUCCESS);
    assert(fifo_put_byte(fifo, ((lorawan_session_config_abp_t*)config)->data_rate) == oss7SUCCESS);
    assert(fifo_put(fifo, ((lorawan_session_config_abp_t*)config)->nwkSKey, 16) == oss7SUCCESS);
    assert(fifo_put(fifo, ((lorawan_session_config_abp_t*)config)->appSKey, 16) == oss7SUCCESS);
    uint32_t dev_addr = __builtin_bswap32(((lorawan_session_config_abp_t*)config)->devAddr);
    assert(fifo_put(fifo, (uint8_t*)&dev_addr, 4) == oss7SUCCESS);
    uint32_t network_id = __builtin_bswap32(((lorawan_session_config_abp_t*)config)->network_id);

    assert(fifo_put(fifo, (uint8_t*)&network_id, 4) == oss7SUCCESS);
  }
  else if(itf_id == ALP_ITF_ID_LORAWAN_OTAA)
  {
    ...
    assert(fifo_put_byte(fifo, control_byte) == oss7SUCCESS);
    assert(fifo_put_byte(fifo, ((lorawan_session_config_otaa_t*)config)->application_port) == oss7SUCCESS);
    assert(fifo_put_byte(fifo, ((lorawan_session_config_otaa_t*)config)->data_rate) == oss7SUCCESS);
    assert(fifo_put(fifo, ((lorawan_session_config_otaa_t*)config)->devEUI, 8) == oss7SUCCESS);
    assert(fifo_put(fifo, ((lorawan_session_config_otaa_t*)config)->appEUI, 8) == oss7SUCCESS);
    assert(fifo_put(fifo, ((lorawan_session_config_otaa_t*)config)->appKey, 16) == oss7SUCCESS);
  }
  else
  {
    assert(fifo_put(fifo, config, config_len) == oss7SUCCESS);
  }
...
void alp_append_return_file_data_action(fifo_t* fifo, uint8_t file_id, uint32_t offset, uint32_t length, uint8_t* data) {
  assert(fifo_put_byte(fifo, ALP_OP_RETURN_FILE_DATA) == oss7SUCCESS);
  assert(fifo_put_byte(fifo, file_id) == oss7SUCCESS);
  alp_append_length_operand(fifo, offset);
  alp_append_length_operand(fifo, length);
  assert(fifo_put(fifo, data, length) == oss7SUCCESS);
}
...
static void append_tag_response(fifo_t* fifo, uint8_t tag_id, bool eop, bool error) {
  ...
  error_t err = fifo_put_byte(fifo, op_return_tag); assert(err == oss7SUCCESS);
  err = fifo_put_byte(fifo, tag_id); assert(err == oss7SUCCESS);
}
...
static void parse_op_return_tag(fifo_t* fifo, alp_action_t* action, bool b6, bool b7) {
  ...
  assert(fifo_pop(fifo, &action->tag_response.tag_id, 1) == oss7SUCCESS);
...
static void parse_op_return_status(fifo_t* fifo, alp_action_t* action, bool b6, bool b7) {
  ...
  uint8_t itf_id;
  assert(fifo_pop(fifo, &itf_id, 1) == oss7SUCCESS);
  ...
  if (itf_id == ALP_ITF_ID_D7ASP)
  {
    ...
    assert(fifo_pop(fifo, (uint8_t*)&interface_status->addressee.id, addressee_len) == oss7SUCCESS);
  }
...
void alp_parse_action(fifo_t* fifo, alp_action_t* action) {
  uint8_t op;
  assert(fifo_pop(fifo, &op, 1) == oss7SUCCESS);
  bool b6 = (op >> 6) & 1;
  bool b7 = op >> 7;
...
void alp_append_tag_request_action(fifo_t* fifo, uint8_t tag_id, bool eop) {
  ...
  assert(fifo_put_byte(fifo, op) == oss7SUCCESS);
  assert(fifo_put_byte(fifo, tag_id) == oss7SUCCESS);
}
...
void alp_append_create_new_file_data_action(fifo_t* fifo, uint8_t file_id, uint32_t length, fs_storage_class_t storage_class, bool resp, bool group) {
  ...
  assert(fifo_put(fifo, (uint8_t*)&header, sizeof(alp_operand_file_header_t)) == oss7SUCCESS);
}
