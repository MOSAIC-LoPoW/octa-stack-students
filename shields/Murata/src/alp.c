...
alp_operand_file_header_t alp_parse_file_header_operand(fifo_t* cmd_fifo) {
  ...
  error_t err = fifo_pop(cmd_fifo, &operand.file_id, 1); assert(err == oss7SUCCESS);
  err = fifo_pop(cmd_fifo, (uint8_t*)&operand.file_header, sizeof(fs_file_header_t)); assert(err == oss7SUCCESS);
...
static void parse_op_return_status(fifo_t* fifo, alp_action_t* action, bool b6, bool b7) {
  ...
  if (itf_id == ALP_ITF_ID_D7ASP)
  {
    ...
    assert(fifo_pop(fifo, interface_status.addressee.id, addressee_len) == oss7SUCCESS);
  }
