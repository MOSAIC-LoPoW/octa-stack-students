...
{
    printINF("***Initializing Nova PM sensor driver***\r\n");

    #ifndef NOVA_PM_CONNECTOR
        printERR("No NOVA_PM_CONNECTOR provided in Makefile\r\n");
        return 0;
    #else
        nova_PM_Header = platform_getHeader((uint8_t)NOVA_PM_CONNECTOR);
        if(!nova_PM_Header.active)
        {
            printERR("Invalid NOVA_PM_CONNECTOR provided in Makefile\r\n");
            return 0;  
        }
        else   
            printINF("Nova PM sensor on P%d, initializing UART\r\n", (uint8_t)NOVA_PM_CONNECTOR);                         
    #endif
    ...
    UART_SetShieldCallback(&nova_PM_receive_data, (uint8_t)NOVA_PM_CONNECTOR);
